...
-- Student Profiles (Corrected table name)
DROP POLICY IF EXISTS "Students can view their own profile" ON public.student_profiles;
DROP POLICY IF EXISTS "Teachers can view students in their tenant" ON public.student_profiles;
DROP POLICY IF EXISTS "Admins can manage students" ON public.student_profiles;
...
BEGIN
    ...
    -- student_profiles table (Corrected table name)
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'student_profiles' AND column_name = 'tenant_id') THEN
        ALTER TABLE public.student_profiles ALTER COLUMN tenant_id TYPE text;
    END IF;
...
-- 6. Re-create the function (Using student_profiles)
...
BEGIN
  ...
  -- Create student profile (Corrected table name)
